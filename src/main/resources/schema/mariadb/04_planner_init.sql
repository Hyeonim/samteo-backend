...
SELECT
  ...
  d.`region_id`,
  d.`district_name`,
  CONCAT(c.`city_name`, ' ', d.`district_name`, ' ', ELT(((d.`district_order` + a.`n`) % 10) + 1, '중앙로', '청년로', '시장길', '해변로', '문화로', '역전로', '공원로', '구암로', '관광로', '상생길'), ' ', 20 + d.`district_order` + a.`n`),
  380000 + (((d.`district_order` * 2 + a.`n` + CHAR_LENGTH(c.`city_id`)) % 8) * 45000),
  100000 + (((d.`district_order` + a.`n`) % 5) * 50000),
...
FROM `tmp_planner_cities` c JOIN `tmp_planner_districts` d ON d.`city_id` = c.`city_id` JOIN `tmp_acc_seq` a
UNION ALL
